...
// Helper for setting point name
void IrisVSTV3AudioProcessor::setPointName(juce::Uuid id, const juce::String& name, bool broadcast)
{
    // SYNC
...
void IrisVSTV3AudioProcessor::addIRFromFileWithID(const juce::File& file, juce::Uuid id)
{
     // Basic checks
...
void IrisVSTV3AudioProcessor::updateParameterNotifiers(juce::String paramId, float value)
{
     // If the host supports parameter changes from plugin, send them.
     // Also, if we have an editor open, it might need repainting if not polling APVTS.
     // APVTS handles `parameterChanged` callbacks if listeners are attached.
     // `store` checks for value change usually? No, `atomic store` just stores.
     // We need to notify APVTS?
     // APVTS parameters are AudioProcessorParameters.
     // We should call `setValueNotifyingHost` on the RangedAudioParameter.
     
     if (auto* p = parameters.getParameter(paramId))
     {
         // Normalize value? The range is 0-1 for most?
         // Our params are float 0-1.
...
// Helper for Listener
void IrisVSTV3AudioProcessor::setListenerPosition(float x, float y, bool broadcast)
{
     if (std::abs(x - currentListenerX) < 0.0001f && std::abs(y - currentListenerY) < 0.0001f) return;
    ...
    for (auto& p : points)
    {
     ...
     if (broadcast && broadcastListener)
          oscManager.setListenerPosition(x, y, this);
}

void IrisVSTV3AudioProcessor::requestFullOSCSync()
{
    oscManager.requestFullSync(this);
}

void IrisVSTV3AudioProcessor::reloadIRChannel(juce::Uuid id, int channel, bool broadcast)
{
    // SYNC
    if (broadcast && broadcastIRs)
...
void IrisVSTV3AudioProcessor::setPointLocked(juce::Uuid id, bool locked, bool broadcast)
{
    // SYNC
...
void IrisVSTV3AudioProcessor::updatePointPosition(juce::Uuid id, float x, float y, bool broadcast)
{
    // SYNC
    // Check if listener
    bool isL = false;
    {
        juce::ScopedLock sl(stateLock);
        for(auto&p : points) if(p.id == id && p.isListener) { isL = true; break; }
    }
    
    if (broadcast)
    {
        if (isL && broadcastListener)
        {
             oscManager.setListenerPosition(x, y, this);
        }
        else if (!isL && broadcastIRs)
        {
...
void IrisVSTV3AudioProcessor::removePoint(juce::Uuid id, bool broadcast)
{
    // SYNC
    if (broadcast && broadcastIRs)
...
juce::Uuid IrisVSTV3AudioProcessor::addWall(float x1, float y1, float x2, float y2, bool broadcast)
{
    juce::ScopedLock sl(stateLock);
    OcclusionWall w;
    w.id = juce::Uuid();
...
void IrisVSTV3AudioProcessor::addWallWithID(juce::Uuid id, float x1, float y1, float x2, float y2)
{
    juce::ScopedLock sl(stateLock);
    // Check existence
...
void IrisVSTV3AudioProcessor::removeWall(juce::Uuid id, bool broadcast)
{
    // SYNC
    if (broadcast && broadcastWalls)
...
void IrisVSTV3AudioProcessor::updateWall(juce::Uuid id, float x1, float y1, float x2, float y2, bool broadcast)
{
    // SYNC
    if (broadcast && broadcastWalls)
...
// void IrisVSTV3AudioProcessor::oscMessageReceived ...
