...
class IrisOSCManager; // Forward Decl
...
class IrisVSTV4AudioProcessor  : public juce::AudioProcessor, 
                                 public juce::Timer,
                                 public juce::AudioProcessorValueTreeState::Listener
{
    friend class IrisOSCManager; // Allow Manager to call private/protected if needed, or just specific setters
public:
    IrisVSTV4AudioProcessor();
    ~IrisVSTV4AudioProcessor() override;
    ...
    // Connection Matrix Edge Set (Symmetric)
    ...
    juce::CriticalSection stateLock; // Protects points, walls, and listeners
    ...
    // setListenerLinked is removed
    void requestFullOSCSync(); // Helper for UI to trigger full sync
    ...
    // New methods
    juce::Uuid addIRFromFile(const juce::File& file); // This one handles its own sync internally or via helper? verify.
    // Sync Helpers
    void addIRFromFileWithID(const juce::File& file, juce::Uuid id); // Remote sync calls this
    void addWallWithID(juce::Uuid id, float x1, float y1, float x2, float y2); // Remote sync
    // .. Wait, addWallWithID is for Receive. addWall is for Local.

    void reloadIRChannel(juce::Uuid id, int channelIndex, bool broadcast = true);
    ...
    // Used to be currentListenerX/Y, now part of NetworkListener struct

    // OSC Data
    std::vector<juce::String> availableIRs; 
    
    // OSC callbacks - Handled by Manager now
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (IrisVSTV4AudioProcessor)
};
