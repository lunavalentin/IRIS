...
juce::AudioProcessorValueTreeState::ParameterLayout IrisVSTV4AudioProcessor::createParameterLayout()
{
...
IrisVSTV4AudioProcessor::IrisVSTV4AudioProcessor()
...
#endif
{
    ...
    // OSC - Managed by Singleton
    // callbacks are via oscManager now
...
const juce::String IrisVSTV4AudioProcessor::getName() const
{
    return JucePlugin_Name;
}

bool IrisVSTV4AudioProcessor::acceptsMidi() const
{
...
bool IrisVSTV4AudioProcessor::producesMidi() const
{
...
bool IrisVSTV4AudioProcessor::isMidiEffect() const
{
...
double IrisVSTV4AudioProcessor::getTailLengthSeconds() const
{
    return 0.0;
}

int IrisVSTV4AudioProcessor::getNumPrograms()
{
    return 1;   // NB: some hosts don't cope very well if you tell them there are 0 programs,
                // so this should be at least 1, even if you're not really implementing programs.
}

int IrisVSTV4AudioProcessor::getCurrentProgram()
{
    return 0;
}

void IrisVSTV4AudioProcessor::setCurrentProgram (int index)
{
}

const juce::String IrisVSTV4AudioProcessor::getProgramName (int index)
{
    return {};
}

void IrisVSTV4AudioProcessor::changeProgramName (int index, const juce::String& newName)
{
}

void IrisVSTV4AudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
...
void IrisVSTV4AudioProcessor::releaseResources()
{
}

#ifndef JucePlugin_PreferredChannelConfigurations
bool IrisVSTV4AudioProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
  #if JucePlugin_IsMidiEffect
...
void IrisVSTV4AudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
...
bool IrisVSTV4AudioProcessor::hasEditor() const
{
    return true;
}

// Helper for setting point name
// Helper for setting point name
void IrisVSTV4AudioProcessor::setPointName(juce::Uuid id, const juce::String& name, bool broadcast)
{
    // SYNC
...
juce::AudioProcessorEditor* IrisVSTV4AudioProcessor::createEditor()
{
    return new IrisVSTV4AudioProcessorEditor (*this);
}


void IrisVSTV4AudioProcessor::getStateInformation (juce::MemoryBlock& destData)
{
    ...
    // Save V4 Network State
    juce::XmlElement* v4Xml = xml.createNewChildElement("V4_STATE");
    v4Xml->setAttribute("localListenerId", localAudioListener.id.toString());
    v4Xml->setAttribute("localListenerName", localAudioListener.name);
    v4Xml->setAttribute("selectedListenerId", selectedListenerId.toString());
    v4Xml->setAttribute("localLocked", localAudioListener.locked);
    
    // Link tracking for remote IDs is no longer locally saved; wait for OSC sync.

    copyXmlToBinary(xml, destData);
}

void IrisVSTV4AudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
    ...
    if (xmlState.get() != nullptr && xmlState->hasTagName("IRIS_STATE"))
    {
        ...
        // Restore V4 Network State
        if (auto* v4Xml = xmlState->getChildByName("V4_STATE"))
        {
            juce::Uuid savedId = juce::Uuid(v4Xml->getStringAttribute("localListenerId"));
            
            bool duplicate = false;
            for (const auto& pair : remoteListeners) {
                if (pair.first == savedId) { duplicate = true; break; }
            }
            ...
            if (!duplicate) {
                ...
                localAudioListener.name = v4Xml->getStringAttribute("localListenerName", "Local Listener");
                oscManager.removeGhostId(oldId); // Clean up the default constructor ghost
            }
            
            juce::Uuid savedSelected = juce::Uuid(v4Xml->getStringAttribute("selectedListenerId"));
            if (savedSelected == savedId && duplicate) {
                selectedListenerId = localAudioListener.id;
            } else {
                selectedListenerId = savedSelected;
            }
            
            localAudioListener.locked = v4Xml->getBoolAttribute("localLocked", false);
            
            // Re-announce ourselves since we potentially changed ID/name during load
            oscManager.setListenerState(localAudioListener.id, localAudioListener.name, localAudioListener.x, localAudioListener.y, false, localAudioListener.locked, this);
        }
...
void IrisVSTV4AudioProcessor::addIRPoint(const juce::String& name)
{
    IRPoint p;
    p.id = juce::Uuid();
    p.name = name;
    ...
    // Color
    juce::Random r(p.id.toString().hashCode());
    p.color = juce::Colour::fromHSV(r.nextFloat() * 0.15f + 0.55f, 0.8f, 0.9f, 1.0f);
    ...
    // SYNC TO MANAGER
    // Since this is a local user action (adding virtual points for testing), we should broadcast it.
    // However, `addIRPoint` is for debugging/testing mainly. 
    // Let's assume we want to sync it.
    // But virtual points have no file...
    // For now, only Local.
    
    if (onStateChanged) onStateChanged();
}

void IrisVSTV4AudioProcessor::addIRFromFileWithID(const juce::File& file, juce::Uuid id)
{
     // Basic checks
    ...
    
    {
        juce::ScopedLock sl(stateLock);
        // Check if ID exists
        for(const auto& p : points) if(p.id == id) return;
    }

    // Logic similar to addIRFromFile but with specific ID
    // Copied logic for brevity, ideally refactor common part.
    // For now I will convert addIRFromFile to use a helper or just duplicate the loading logic 
    // OR calling addIRFromFile and then changing ID is risky if ID is used for map keys immediately.
    // Actually, `addIRFromFile` allocates a NEW UUID. 
    // We should refactor `addIRFromFile` to take optional ID.
    
    // Instead of refactoring the whole big function, I will call a private helper or copy-paste the loading block roughly.
    // To be safe and clean, I will implement a private `loadIRInternal(file, id)` and make `addIRFromFile` call it.
    // But I can't change header privately easily now without another edit.
    // I will implement the loading logic here again (it's safe).
    
    // ... Copying loading logic ...
    // Wait, better to just modify the ID after creation? 
    // `addIRFromFile` returns UUID. 
    // If I call it, it creates a point with random UUID.
    // Then I can find it and change UUID to `id`.
    // BUT `updateWeightsGaussian` uses UUID maps...
    // So changing UUID after might leave garbage in maps?
    // Yes, `targetWeights[id]` etc.
    // So I MUST Create with correct ID.
    
    // I will implementation `loadIR` properly.
    // To avoid massive code duplication in this tool call, I will delegate.
    
    // Actually, I will implement it by modifying `addIRFromFile` to be `addIRFromFile(file, knownID)` in a moment?
    // No, I will just copy the logic effectively or use a const_cast trick (ugly).
    // Correct way: Refactor loading.
    // I'll assume I can copy the loading logic here.
    
    // ... (Loading logic omitted for brevity in thought, but will be in tool call) ...
    
    // Actually, I can use a simpler approach: 
    // The `addIRFromFile` function is big.
    // I will just Refactor `addIRFromFile` to take `juce::Uuid id = juce::Uuid::null()`?
    // I cannot change signature without header change.
    // I added `addIRFromFileWithID` in header.
    ...
    // ... V2 Processing (simplified for sync? No, must be same) ...
    // ...
    // (I will include the processing logic in the replacement)
    
    // NEW POINT
    IRPoint p;
    p.id = id; // USE PASSED ID
    ...
    int storedOnset = 0;
    {
        ...
        for (int c = 0; c < numChannels; ++c) globalPeak = juce::jmax(globalPeak, wholeBuffer->getMagnitude(c, 0, numSamples));
        float threshold = globalPeak * 0.1f;
        int onset = 0;
        for (int i = 0; i < numSamples; ++i) {
            for (int c = 0; c < numChannels; ++c) {
                if (std::abs(wholeBuffer->getSample(c, i)) > threshold) { onset = i; break; }
            }
            if (onset != 0) break;
        }
        storedOnset = onset;
        if (onset > 0) {
            ...
            if (fadeLen > 0 && fadeLen < newNumSamps) wholeBuffer->applyGainRamp(0, 0, fadeLen, 0.0f, 1.0f);
        }
        float rms = 0.0f;
        for (int c = 0; c < numChannels; ++c) rms += wholeBuffer->getRMSLevel(c, 0, wholeBuffer->getNumSamples());
        rms /= (float)numChannels;
        if (rms > 0.00001f) { storedGain = 0.1f / rms; wholeBuffer->applyGain(storedGain); }
    }
...
juce::Uuid IrisVSTV4AudioProcessor::addIRFromFile(const juce::File& file)
{
    // Basic checks
...
void IrisVSTV4AudioProcessor::updateConvolver(IRPoint& p)
{
...
// Helper for PARAMETER Sync
void IrisVSTV4AudioProcessor::updateParameterNotifiers(juce::String paramId, float value)
{
...
void IrisVSTV4AudioProcessor::parameterChanged(const juce::String& parameterID, float newValue)
{
    if (!isUpdatingFromOSC && broadcastGlobals) {
...
// Helper for Listener
void IrisVSTV4AudioProcessor::updateListenerPosition(juce::Uuid id, float x, float y, bool broadcast)
{
    juce::ScopedLock sl(stateLock);
    
    NetworkListener* movedListener = nullptr;
    if (id == localAudioListener.id) {
        movedListener = &localAudioListener;
    } else if (remoteListeners.count(id)) {
        movedListener = &remoteListeners[id];
    }
    
    if (!movedListener) return;
    if (movedListener->locked && broadcast) return; // Prevent movement if locked, allow OSC sync

    movedListener->x = juce::jlimit(0.0f, 1.0f, x);
    movedListener->y = juce::jlimit(0.0f, 1.0f, y);

    if (broadcast) {
        // Find all connected nodes using BFS
        std::set<juce::String> visited;
        std::queue<juce::String> queue;
        visited.insert(movedListener->id.toString());
        queue.push(movedListener->id.toString());
        ...
        // Move all connected nodes that are not locked
        for (const auto& uIdStr : visited) {
            juce::Uuid uId(uIdStr);
            if (uId == id) continue; // Already moved
            ...
            if (follower && !follower->locked) {
                follower->x = movedListener->x;
                follower->y = movedListener->y;
                // Broadcast this follower (legacy linkedFlag is hardcoded to false)
                oscManager.setListenerState(follower->id, follower->name, follower->x, follower->y, false, follower->locked, this);
            }
        }
        
        // Broadcast the master moving listener
        oscManager.setListenerState(movedListener->id, movedListener->name, movedListener->x, movedListener->y, false, movedListener->locked, this);
    }
    if (onStateChanged) onStateChanged();
}

void IrisVSTV4AudioProcessor::toggleLinkMatrix(juce::Uuid id1, juce::Uuid id2, bool broadcast)
{
    juce::ScopedLock sl(stateLock);
    ...
    if (isLinking) {
        ...
        // Enforce Transitivity Step: Build connected components and make them cliques
        ...
        std::set<juce::String> visited;
        std::vector<std::vector<juce::String>> comps;
        ...
        // Rebuild link matrix as cliques
        linkMatrix.clear();
        ...
        // Unlinking: completely sever s1 from s2's group.
        std::map<juce::String, std::set<juce::String>> adj;
        for (const auto& e : linkMatrix) {
            adj[e.first].insert(e.second);
            adj[e.second].insert(e.first);
        ...
        // Find s2's fully connected group, excluding s1
        ...
        // Remove ALL edges between s1 and any node in s2's group
        for (const auto& nodeInS2Group : s2Group) {
            std::pair<juce::String, juce::String> eToRemove(std::min(s1, nodeInS2Group), std::max(s1, nodeInS2Group));
            linkMatrix.erase(eToRemove);
        }
    }
    ...
    if (broadcast) {
        oscManager.syncLinkMatrix(this);
    }
}

void IrisVSTV4AudioProcessor::setLinkMatrixConnections(const std::vector<std::pair<juce::String, juce::String>>& edges)
{
    juce::ScopedLock sl(stateLock);
    linkMatrix.clear();
...
void IrisVSTV4AudioProcessor::setListenerLocked(juce::Uuid id, bool locked, bool broadcast)
{
    juce::ScopedLock sl(stateLock);
    if (id == localAudioListener.id) {
    ...
    if (broadcast) {
        if (id == localAudioListener.id) {
            oscManager.setListenerState(id, localAudioListener.name, localAudioListener.x, localAudioListener.y, false, locked, this);
        } else if (remoteListeners.count(id)) {
...
void IrisVSTV4AudioProcessor::requestFullOSCSync()
{
...
void IrisVSTV4AudioProcessor::setPointLocked(juce::Uuid id, bool locked, bool broadcast)
{
    // SYNC
    if (broadcast && broadcastIRs) // Locked applies to IRs and Walls, simplified to IRs flag or add more granular?
...
void IrisVSTV4AudioProcessor::updatePointPosition(juce::Uuid id, float x, float y, bool broadcast)
{
    // SYNC
    if (broadcast && broadcastIRs)
...
void IrisVSTV4AudioProcessor::removePoint(juce::Uuid id, bool broadcast)
{
    // SYNC
    if (broadcast && broadcastIRs)
...
juce::Uuid IrisVSTV4AudioProcessor::addWall(float x1, float y1, float x2, float y2, bool broadcast)
{
    juce::ScopedLock sl(stateLock);
    OcclusionWall w;
    w.id = juce::Uuid();
...
void IrisVSTV4AudioProcessor::addWallWithID(juce::Uuid id, float x1, float y1, float x2, float y2)
{
    juce::ScopedLock sl(stateLock);
    // Check existence
    for(const auto& existing : walls) if(existing.id == id) return;
...
void IrisVSTV4AudioProcessor::removeWall(juce::Uuid id, bool broadcast)
{
    // SYNC
    if (broadcast && broadcastWalls)
...
void IrisVSTV4AudioProcessor::updateWall(juce::Uuid id, float x1, float y1, float x2, float y2, bool broadcast)
{
    // SYNC
    if (broadcast && broadcastWalls)
...
void IrisVSTV4AudioProcessor::constrainPointToWalls(float& x, float& y)
{
    juce::ScopedLock sl(stateLock);
...
void IrisVSTV4AudioProcessor::updateWeightsGaussian()
{
    // Target Listener coordinates for convolution
    NetworkListener* listener = &localAudioListener;
    ...
    for (auto& p : points) {
        ...
        {
            float ix, iy, t_wall;
            // Listener (currentX,Y) to Point (p.x, p.y)
            if (getIntersectionPoint(localAudioListener.currentX, localAudioListener.currentY, p.x, p.y, 
...
void IrisVSTV4AudioProcessor::timerCallback()
{
    ...
    // Apply to ALL listeners so remote viewers also see smooth movement
    auto applyPhysics = [&](NetworkListener& listener) {
        ...
        if (std::abs(targetX - listener.currentX) > 0.0001f)
            listener.currentX += (targetX - listener.currentX) * physicsAlpha;
        if (std::abs(targetY - listener.currentY) > 0.0001f)
            listener.currentY += (targetY - listener.currentY) * physicsAlpha;
    };
...
void IrisVSTV4AudioProcessor::sendWeightsOSC()
{
...
// oscMessageReceived moved to Manager
// void IrisVSTV4AudioProcessor::oscMessageReceived ...



juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter()
{
    return new IrisVSTV4AudioProcessor();
}

void IrisVSTV4AudioProcessor::loadLayoutFromJSON(const juce::File& file)
{
    if (!file.existsAsFile()) return;
...
void IrisVSTV4AudioProcessor::saveLayoutToJSON(const juce::File& file)
{
    juce::ScopedLock sl(stateLock);
